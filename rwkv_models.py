...
class RWKV_model(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.lm_head = nn.Linear(config.n_embd, config., bias=False)
    ...
    def forward(self, tok_emb, targets=None):
        '''
        toke_emb shape: (bsz, seq_len, dim)
        '''
...
if __name__ == "__main__":
    ...
    gptconf = GPTConfig(
        ...
        n_layer = 2, n_head = 8, n_embd = 1024, # size of the model
        dropout = 0, # for determinism
        bias = bias,
        )
    
    model = RWKV_model(gptconf)
    test_input = torch.randn(4, 100, 1024)
